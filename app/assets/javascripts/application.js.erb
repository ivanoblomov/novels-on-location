...
nOL.init = function(settings) {
  ...
  FB.init({appId:settings['fbAppId'],cookie:true,status:true,version:'v2.11',xfbml:true});
...
nOL.promptForNotes = function(id, notes) {
  var value = prompt('Add anything noteworthy. Write about the climactic scene that took place here, for example, or the real-life landmark a setting from the story was modeled on.', unescape(notes || ''));
...
nOL.myBookmark = function(location) {
  return location.bookmark_user_ids && location.bookmark_user_ids.indexOf(nOL.fb_session.userID) != -1
}
...
nOL.showPins = function(keyword, path) {
  ...
  if (keyword == '')
    ...
    for (const location of nOL.locations) {
      if (regex.test(Reflect.get(location, anchorKindToLocationProperty[nOL.anchorKind])))
        nOL.showPin(location.id);
    }
    nOL.map.fitBounds(nOL.bounds);
...
nOL.closeBalloon = function() {
  if (nOL.openWindow != undefined)
    nOL.openWindow.close();
}
...
nOL.openBalloon = function(location) {
  ...
  if (location.user_id == null && location.user_token == null) html += '<input onClick="nOL.claimPin(\'' + location.id + '\')" type="button" value="Claim" title="Claim this pin"/>';
...
nOL.tagLinks = function(location) {
  var tags = $.map(
    location.tags.split(','), function(tag, i) {
      ...
      var link = '<a href="' + tagPath + '" onClick="nOL.showPins(\'' + escape(tag) + '\', \'' + tagPath + '\')" title="Show all locations with the tag ' + escape(tag) + '">' + tag + '</a>';
      return link;
    }
  );
  return tags.join(', ');
