...
module FixtureAccess
  def self.extended(base)
    ...
    fixtures_folder = Rails.root.join('spec/fixtures').to_s
      ...
      fixtures.each do |table_name|
        ...
        define_method(table_name) do |*fixture_symbols|
          ...
          instances = fixture_symbols.map do |fixture_symbol|
            if (fix = ActiveRecord::FixtureSet.cached_fixtures(ActiveRecord::Base.connection,
                                                               table_name).first.fixtures[fixture_symbol.to_s])
